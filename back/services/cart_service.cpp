...
CartService::CartService(PgPool &pool) : pool_(pool) {}
...
int CartService::getOrCreateCart(int userId) {
  ...
  conn->exec_params("INSERT INTO carts (user_id) VALUES ($1) ON CONFLICT "
                    "(user_id) DO NOTHING",
                    {std::to_string(userId)});

  auto res = conn->exec_params("SELECT id FROM carts WHERE user_id = $1",
                               {std::to_string(userId)});
...
void CartService::addToCart(int userId, int productId, int quantity) {
  ...
  auto productRes = conn->exec_params(
      "SELECT stock FROM products WHERE id = $1", {std::to_string(productId)});
  ...
  auto existRes = conn->exec_params(
      "SELECT quantity FROM cart_items WHERE cart_id = $1 AND product_id = $2",
      {std::to_string(cartId), std::to_string(productId)});

  int alreadyInCart =
      (existRes.GetRows() > 0) ? std::stoi(existRes.GetEl(0, 0)) : 0;
  ...
  conn->exec_params("UPDATE carts SET updated_at = NOW() WHERE id = $1",
...
void CartService::updateQuantity(int userId, int productId, int quantity) {
  ...
  if (stock < quantity) {
    throw std::runtime_error(
        "Not enough stock available (requested: " + std::to_string(quantity) +
        ", in stock: " + std::to_string(stock) + ")");
  }
  ...
  auto updateRes =
      conn->exec_params("UPDATE cart_items SET quantity = $1 "
...
void CartService::removeItem(int userId, int productId) {
  ...
  if (!cartId.has_value())
    return; // no cart exists — nothing to remove
  PgConnGuard conn(pool_);
  conn->exec_params(
...
json::object CartService::getCart(int userId) {
  ...
  auto totalRes = conn->exec_params("SELECT CAST(COALESCE(SUM(p.price * "
                                    "ci.quantity), 0) AS NUMERIC(10,2))::text "
                                    "FROM cart_items ci "
                                    "JOIN products p ON p.id = ci.product_id "
                                    "WHERE ci.cart_id = $1",
                                    {std::to_string(cartId)});
...
json::object CartService::checkout(int userId) {
  ...
  try {
    ...
    // Snapshot cart inside the transaction and lock rows against concurrent
    // edits
    auto itemsRes =
        conn->exec_params("SELECT product_id, quantity FROM "
                          "cart_items WHERE cart_id = $1 FOR UPDATE",
                          {std::to_string(cartId)});
    ...
    for (int i = 0; i < itemsRes.GetRows(); i++) {
      ...
      auto stockRes = conn->exec_params(
      ...
      if (stock < quantity) {
        conn->rollback();
        throw std::runtime_error("Insufficient stock for product " +
                                 std::to_string(productId) +
    ...
    auto orderRes = conn->exec_params(
        "INSERT INTO orders (user_id, total, status) "
        "SELECT $1, CAST(SUM(p.price * ci.quantity) AS NUMERIC(10,2)), "
        "'pending' "
    ...
    for (const auto &item : items) {
      conn->exec_params("INSERT INTO order_items "
                        "(order_id, product_id, price_at_purchase, quantity) "
                        "VALUES ($1, $2, $3, $4)",
                        {std::to_string(orderId),
                         std::to_string(item.productId), item.priceStr,
                         std::to_string(item.quantity)});

      conn->exec_params(
          "UPDATE products SET stock = stock - $1 WHERE id = $2",
          {std::to_string(item.quantity), std::to_string(item.productId)});
    }

    // ── Step 4: clear the cart ────────────────────────────────────────────
    conn->exec_params("DELETE FROM cart_items WHERE cart_id = $1",
                      {std::to_string(cartId)});
