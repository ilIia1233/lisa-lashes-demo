...
ResourceRepository *ResourceContext::resourceService = nullptr;
...
std::vector<Resource> ResourceRepository::getAllResources() {
  ...
  // Fetch all resources
  auto resData = db.exec_params("SELECT id, name, active "
                                "FROM resources "
                                "ORDER BY id ASC",
                                {});
  ...
  auto svcData = db.exec_params(
      "SELECT rs.resource_id, ss.id, ss.name "
...
void ResourceRepository::updateResource(int id, json::object obj) {
  ...
  db.exec_params("UPDATE resources SET "
...
void ResourceRepository::deleteResource(int id) {
  db.exec_params("DELETE FROM resources WHERE id = $1::integer",
                 {std::to_string(id)});
}
