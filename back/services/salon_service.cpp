...
std::vector<SalonService> SalonServiceRepository::getAllServices() {
  ...
  auto res = conn->exec_params("SELECT id, name, duration, price, active "
                               "FROM salon_services "
                               "ORDER BY id ASC",
                               {});
...
void SalonServiceRepository::addService(const std::string &name,
  ...
  PgConnGuard conn(pool_);
  conn->exec_params(
      "INSERT INTO salon_services (name, duration, price, active) "
      "VALUES ($1, $2, $3, true)",
      {name, duration, price});
}
...
void SalonServiceRepository::deleteService(int id) {
  PgConnGuard conn(pool_);
  conn->exec_params("DELETE FROM salon_services WHERE id = $1::integer",
                    {std::to_string(id)});
}
...
void SalonServiceRepository::setServicesForResource(
  ...
  PgConnGuard conn(pool_);
  conn->exec_params("DELETE FROM resource_services "
